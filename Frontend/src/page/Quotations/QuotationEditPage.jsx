...
function QuotationEditPage() {
    ...
    useEffect(() => {
        if (quotationData) {
            // Map over quotationData.products and transform each product
            const formattedProducts = quotationData.products.map(product => {
                return {
                    name: product.qproduct_name,
                    brand: product.qproduct_brand,
                    part_number: product.qproduct_part_number,
                    price1: product.qproduct_unit_price,
                    quantity: product.qproduct_quantity,
                    discount: product.qproduct_discount,
                    total: product.qproduct_total
                };
            });

            const formattedServices = quotationData.services.map(service => {
                return {
                    type: service.qservice_type_of_service,
                    description: service.q_service_description,
                    time: service.qservice_time,
                    // Add more properties as needed
                };
            });
            setTableData(formattedProducts);
            setShowServiceTable(formattedServices);
